  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [pets, pendingPets, lostActive, pendingLost, usersCount, staffCount, petRaw, vaxRaw] = await Promise.all([
        ...
        supabase.from("profiles").select("count", { count: "exact", head: true }).eq("role", "Staff"),
        supabase.from("pets").select("species, created_at"),
        supabase.from("vaccinations").select("count", { count: "exact", head: true }).gte("created_at", new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString())
      ]);
      ...
      if (mounted) {
        ...
        setRecentUsers((recentUserData ?? []).map((u: any) => ({
             ...u,
