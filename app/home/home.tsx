  ...
  useEffect(() => {
    async function loadPosts() {
      ...
      if (data && !error) {
        ...
        const formatted = data.map((d: Post & { post_comments?: Comment[] }) => ({
          ...d,
  ...
  return (
    <div className="home-page">
      ...
      <nav className="mobile-bottom-nav">
        ...
        <button type="button" className="bottom-nav-item" onClick={() => router.push("/pets")}>
          <IconPaw size={22} />
          My Pets
        </button>
        <button type="button" className="bottom-nav-item" onClick={() => router.push("/lost-pets")}>
          <IconAlertTriangle size={22} />
          Lost Pets
        </button>
        <button type="button" className="bottom-nav-item" onClick={() => router.push("/pets?tab=vaccinations")}>
          <IconSyringe size={22} />
          Vaccines
        </button>
